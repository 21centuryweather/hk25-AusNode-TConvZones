...
def detrend_rolling_window(da, window_size=30):
    ...
    smoothed_data = (
        padded_data.rolling(time=window_size, center=True)
        .mean("time")
        .isel(time=slice(int(window_size / 2), -int(window_size / 2)))
    )
    return da - smoothed_data


def calc_anom(input_da, base_clim):
...
def calc_anom_annual(input_da, base_clim):
...
def calc_trend3d(da: xr.DataArray, dim: str):
    ...
    return xr.apply_ufunc(
        calc_trend1d, da, input_core_dims=[[dim]], vectorize=True, dask="parallelized"
    )
